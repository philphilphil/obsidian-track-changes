...
4. **Commands**: open panel, finalize for publish (`src/finalize.ts`), five manual-authoring commands (`src/authoring.ts`), accept/reject change at cursor (`findChangeAt` in `src/operations.ts`; cursor must be inside or touching the mark, no next-change fallback).
