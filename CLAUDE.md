...
4. **Commands**: open panel, finalize for publish (`src/finalize.ts`), five manual-authoring commands (`src/authoring.ts`), accept/reject change, remove highlight, delete comment at cursor (`editsAtCursor` in `src/operations.ts`; cursor must be inside or touching the mark, no next-mark fallback; anchor highlights are skipped, deleting a thread's only message takes an anchor *highlight* (never a change), and accept/reject take the change's anchored thread, as in the panel).
...
A thread is a run of `{>>…<<}` blocks with only inline whitespace (no blank line) between them in the same paragraph. First is root, rest are replies. A `{==…==}` highlight sitting directly before the root (only spaces/tabs between) is the thread's **anchor**: `Thread.anchorIndex` points at it, `anchorNodeIndexes(parsed)` collects them, and the panel renders it as a quoted span *inside* the thread card instead of emitting its own highlight card (and leaves it out of the header's highlight count) — issue #41, so `{==text==}{>>comment<<}` (what `authoring.ts` writes for a comment on a selection, and the CriticMarkup convention) is one review entry, not two. `thread.from`/`to` deliberately still cover the comments alone, so `deleteThread` can never swallow anchored prose; the panel adds an explicit `removeHighlight(anchor)` edit when the whole thread (or its last remaining message) goes. The merged card offers no "Remove highlight" of its own — unanchoring happens by editing the markup (deliberate: the card is one review entry with one set of actions). An addition/deletion/substitution directly before the root anchors the same way (issue #45; `aitext` never anchors): the thread is the suggestion's rationale, rendered inside the *suggestion* card (diff, accept/reject, messages, reply box; no "Delete thread"). `changeThreads(parsed)` maps change → thread; `anchorNodeIndexes` stays highlight-only. **The asymmetry is a hard rule:** a highlight anchor goes when its thread (or last message) goes, a change anchor never does — deleting comments must never resolve a change. Conversely `resolveChange` (panel accept/reject and `editsAtCursor`) folds the change's anchored thread into a single edit spanning `[change.from, thread.to)`, so rebase applies or drops the pair together (a half-applied pair could re-anchor the orphaned thread on a neighbouring mark). `appendComment(source, change, …)` inserts a new attributed thread at `change.to` (`before` = up to 40 chars of context plus `change.raw`, so a stale insert can't relocate onto an identical mark) so the parser immediately anchors it; the panel's "Comment" action on thread-less suggestion cards uses it. Change-anchored threads aren't counted as "comments" in the header but still consume a thread number, so panel `#N` matches the inline chips. The prefix lives outside the `>>`/`<<` delimiters, so adjacency detection is unchanged. Authorship resolves by precedence: `metaAuthor` (the `author="…"` prefix, on all six kinds) → legacy `<Name>:` body prefix (comments only — single token, alpha-leading, ≤30 chars, see `src/authors.ts`; always stripped from `text` even when `metaAuthor` is present) → `localAuthorName` setting → "You". Comments accept **both** the new `author="…"` form and the legacy `{>>Name: text<<}` form. Replies are no longer the only mark the plugin writes — manual authoring (`src/authoring.ts`, five commands) also writes marks, and both paths share the same attribution contract via `buildAttributionPrefix`: always `date="<stamp>"` and, when `localAuthorName` is set, `author="<sanitized name>"` — so a mark with no `author=` (or the user's configured name) is the user's, whether it came from a reply or a manual-authoring command. `date` is display-only, so the stamp can be date-only (`2026-06-14`) or a full second-precision ISO timestamp (`2026-06-14T12:23:46Z`) and both render. Treat this precedence as a hard contract; don't add other heuristics. Keep this consistent with `docs/SKILL.md`.
