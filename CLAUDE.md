...
4. **Commands**: open panel, finalize for publish (`src/finalize.ts`), five manual-authoring commands (`src/authoring.ts`), toggle/cancel change-tracking session (`src/session.ts`, `src/trackdiff.ts`) plus the status-bar tracking indicator.
