...
- `src/parser.ts` scans source text and emits a `ParseResult` with `nodes` (the six node kinds — the five CriticMarkup forms plus `aiLabel`) and `threads` (adjacent `{>>…<<}` blocks group). Comments expose `authorName: string | null` — the captured `<Name>:` prefix (original casing) or `null` if unprefixed. A `{>>ai<<}` comment whose whole body is the keyword `ai` (case-insensitive, see `isAiLabelBody`) parses as a separate `aiLabel` node — a passive "AI-written" marker rendered as an "AI" pill, excluded from threads and the panel, stripped on finalize. **Code blocks are skipped** — markup inside fenced (```` ``` ````, `~~~`), indented (4-space / tab), or inline-backtick code is left alone.
