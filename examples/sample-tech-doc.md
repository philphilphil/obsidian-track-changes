...
> each comment and suggestion as a card. The reviewer prefix here is
> `AI:` — change it in settings if your agent uses a different one.

Search hits the `recipes` table on every keypress. {>>AI: How many queries per second at peak today? The right strategy is very different at 100 qps versus 10k qps — name a number.<<} We've been talking about adding a cache layer but kept deferring it; this doc settles on an approach.
